...
const opt = {
  ...
  curl: (q.get('curl') ?? 'roll') as CurlType,
...
app.innerHTML = `
  <div class="wrap">
    <header>
      ...
      <div class="sub">${kind === 'pdf' ? 'PDF source (pdf.js)' : 'Image source'}, forced <code>renderer: '${renderer}'</code>. Options are reactive (they reload with a fresh book).</div>
