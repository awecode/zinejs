...
| `curl` | `'roll' \| 'cone' \| 'simple' \| 'fold' \| 'peel'` | `'roll'` | Page-curl model (WebGL2 only — see [Curl models](#curl-models)). |
...
For anchored models (`cone`, `fold`, `peel`), the fold originates at the corner nearest where the reader taps/grabs.
